...
public class DFABuilder {
    ...
    public DFABuilder(TemporalInvariantSet NIFbys,
    ...
    /*
     * Returns a character encoding for the given EventType. Updates the
     * encodings map and possibly INITIAL/TERMINAL characters if this EventType
     * has not been seen before. TODO: Address possibility of encoding
     * collisions
     */
    ...
    private Automaton buildInitialDFAModel(TemporalInvariantSet NIFbys) {
        ...
        Automaton model = intersectModelWith(BasicAutomata.makeAnyString(),
                NIFbys);
    ...
    /*
     * Builds the final DFAModel from the initial model and the given set of
     * mined invariants.
     */
    private Automaton buildFinalModel(TemporalInvariantSet minedInvariants) {
        Automaton finalModel_ = intersectModelWith(initialModel.clone(),
                minedInvariants);
        finalModel_.minimize();
        return finalModel_;
    }

    /* Intersects the given model with DFAs for all of the given invariants */
    private Automaton intersectModelWith(Automaton model,
    ...
    /*
     * Returns a regular expression describing the INITIAL and TERMINAL
     * constraints for the DFA.
     */
     ...
     * @throws IOException
     */
    public void exportDotAndPng(String filename, boolean exportFinal)
            throws IOException {
