...
public class TraceParserTests extends SynopticTest {
	...
	private ChainsTraceGraph genExpectedGraphForBasicMultipleRelation(
		...
		expectedGraph.tagInitial(cMain,
				Arrays.asList(defRelation, callRelation));
	...
	private ChainsTraceGraph genExpectedGraphForMultipleRelationsPerLine(
		...
		expectedGraph.tagInitial(mainMain,
				Arrays.asList(defRelation, callRelation, staticRelation));

		EventNode mainInstance = events.get(1);
		mainMain.addTransitions(mainInstance,
				Arrays.asList(defRelation, callRelation, instanceRelation));

		EventNode mainMain2 = events.get(2);
		mainInstance.addTransitions(mainMain2,
				Arrays.asList(defRelation, returnRelation, staticRelation));

		EventNode fooFoo = events.get(3);
		mainMain2.addTransitions(fooFoo,
				Arrays.asList(defRelation, callRelation, nativeRelation));

		EventNode mainMain3 = events.get(4);
		fooFoo.addTransitions(mainMain3,
				Arrays.asList(defRelation, returnRelation, staticRelation));
	...
	private ChainsTraceGraph genExpectedGraphForClosureAndGenericMultipleRelations(
		...
		EventNode mainMain2 = events.get(2);
		mainInstance.addTransitions(mainMain2,
				Arrays.asList(defRelation, returnRelation));
