...
public class CheckTraceParserAgainstOldParsers {
	...
	private static List<TraceParser.Occurrence> readGraphSet(TraceParser m, String baseName,
			int count, int linesToRead) throws ParseException, InternalSynopticException {
	...
	@Test
	public void testPeterson() throws IOException, ParseException, InternalSynopticException {
		TimedTask oldTimer = new TimedTask("old");
	...
	@Test
	public void testStenning() throws IOException, ParseException, InternalSynopticException {
		TimedTask oldTimer = new TimedTask("old");
		GraphBuilder b = new GraphBuilder();
	...
	// Comparison of messages based on label. Passed into Graph.equalsWith() below
	@Ignore("HelperClass")
	public class MessageEquality implements Predicate.IBinary<MessageEvent, MessageEvent> {
	 ...
	 */
	private void checkGraphs(String prefix, Graph<MessageEvent> g1, Graph<MessageEvent> g2) {
...
//			System.out.println("head-count: " + g1.getInitialNodes().size() + " vs " + g2.getInitialNodes().size());
//		} 
