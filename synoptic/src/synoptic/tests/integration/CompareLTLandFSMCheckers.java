...
public class CompareLTLandFSMCheckers {
	 ...
	 * (using this parser) and then model checking the resulting graphs
	 * for invariants using both types of model checkers.
	 */
	TraceParser parser;
	 ...
	 * checkers.
	 *
	 * @param <T> types of nodes in the graph
	 * @param invs the invariants for which counter-examples should be produced
	 * @param graph the graph over which counter-examples should be produced
	 */
	private <T extends INode<T>> void compareViolations(List<ITemporalInvariant> invs, IGraph<T> graph) {
		...
		for (int i = 0; i < invs.size(); i++) {
			...
			RelationPath<T> fsm_path = FsmModelChecker.getCounterExample(inv, graph);
