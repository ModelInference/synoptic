...
import synoptic.invariants.fsmcheck.constraints.ConstrainedFsmModelChecker;
 ...
 */
public class ConstrainedTemporalInvariantSet implements
        Iterable<ITemporalInvariant> {
    private static Logger logger = Logger
            .getLogger("ConstrainedTemporalInvSet Logger");

    LinkedHashSet<ITemporalInvariant> invariants = new LinkedHashSet<ITemporalInvariant>();

    public ConstrainedTemporalInvariantSet() {
     ...
     * @param invariants
     */
    public ConstrainedTemporalInvariantSet(Set<ITemporalInvariant> invariants) {
    ...
    public void add(ConstrainedTemporalInvariantSet set) {
    ...
    public static <T extends INode<T>> CExamplePath<T> getCounterExample(
        ...
        try {
            return ConstrainedFsmModelChecker.getCounterExample(
                    (TempConstrainedInvariant<?>) inv, g);
        } finally {
            refinement.stop();
        }
    }
    ...
    public <T extends INode<T>> List<CExamplePath<T>> getAllCounterExamples(
        ...
        try {
            ...
            for (ITemporalInvariant tinv : invariants) {
                CExamplePath<T> path = ConstrainedFsmModelChecker
                        .getCounterExample((TempConstrainedInvariant<?>) tinv,
                                graph);
                if (path != null) {
                    paths.add(path);
                }
     ...
     */
    public boolean sameInvariants(ConstrainedTemporalInvariantSet set2) {
    ...
    private static <T extends INode<T>> void printStats(IGraph<T> g,
            ConstrainedTemporalInvariantSet overapproximatedInvariantsSet,
