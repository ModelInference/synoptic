...
public class PettersonTest {
	public static void main(String[] args) throws Exception {
		...
		e.exportAsDotAndPng("output/peterson/synoptic.invariants-AP.dot", s
				.getInvariantGraph("AP"));
		e.exportAsDotAndPng("output/peterson/synoptic.invariants-AFby.dot", s
				.getInvariantGraph("AFby"));
		e.exportAsDotAndPng("output/peterson/synoptic.invariants-NFby.dot", s
	...
	private static void exportSCCsWithInvariants(GraphVizExporter e,
		...
		for (Set<Partition> scc : sccs) {
			...
			e.exportAsDotAndPngFast("output/peterson/partition-"+partN+"-synoptic.invariants.dot", s2
					.getInvariantGraph("AP"));
	...
	private static void mineSplitInvariants(Graph<MessageEvent> g,
		...
		for (String key : buckets.keySet()) {
			for (Entry<String, Set<MessageEvent>> e : buckets.get(key)
				...
				exporter.exportAsDotAndPng("output/peterson/synoptic.invariants-node"+key+"-round"
